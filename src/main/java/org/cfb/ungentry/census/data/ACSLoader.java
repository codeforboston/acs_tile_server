...
public class ACSLoader {
	...
	public static class TopologyProcessor extends RecursiveTileProcessor {
		...
		public void process(TileElement iTile) {
				...
				FeatureCollection aReducedFeat = iTile.collection;
				System.out.println("Nb of shapes"+ aReducedFeat._shapes.size() );
				Topology aTopo =  TopojsonApi.featureToTopology(aReducedFeat, _map, "MAP");
				
				String aSavingName = _saving_path + File.separator + "map" + File.separator +
						 iTile.zoom + File.separator +
						 aTopo._meta_properties.get("x") + File.separator  ;
				
				new File(aSavingName).mkdirs();
				...
				/*
				for (Feature aShape:aReducedFeat._shapes.values()) {
						aShape.fill(_display, _colorifier);
				}
				
				for (Feature aShape:aReducedFeat._shapes.values()) {		
					aShape.getBounding().draw(_display, new Color(0,0,255));
				}
				*/
				...
				String png = aSavingName+ aTopo._meta_properties.get("y") + ".png";
				
				_display.saveImage(png);
	...
	public static void buildTopoJson(String iSavingPath, String iGeoCSVFile, String iState, String iField){
		...
		try {
			...
			FeatureCollection.processTiles(aFeat, 1, 12, aProcessor);
